...
func New(scheme SchemeType, _ confmap.ProviderSettings) confmap.Provider {
	...
	return &provider{scheme: scheme, ctx: ctx, cancel: cancel}
}
...
func (p *provider) createClient() (*http.Client, error) {
	...
	default:
		return nil, fmt.Errorf("invalid scheme type: %s", p.scheme)
	}
}

func (p *provider) Retrieve(ctx context.Context, uri string, watcherFunc confmap.WatcherFunc) (*confmap.Retrieved, error) {
	if !strings.HasPrefix(uri, string(p.scheme)+":") {
		return nil, fmt.Errorf("%q uri is not supported by %q provider", uri, string(p.scheme))
	}
