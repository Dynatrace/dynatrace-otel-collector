	...
	tcs := []struct {
	...
	}{
		{
			...
			attrs: map[string]oteltest.ExpectedValue{
				...
				"dt.entity.kubernetes_cluster": oteltest.NewExpectedValue(oteltest.AttributeMatchTypeEqual, testNs),
				"dt.kubernetes.workload.kind":  oteltest.NewExpectedValue(oteltest.AttributeMatchTypeEqual, "job"),
				"dt.kubernetes.workload.name":  oteltest.NewExpectedValue(oteltest.AttributeMatchTypeEqual, "telemetrygen-"+testID+"-traces-job"),
				"k8s.cluster.name":             oteltest.NewExpectedValue(oteltest.AttributeMatchTypeEqual, testNs),
